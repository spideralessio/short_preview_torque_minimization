...
for i = 1:count
    ...
    Q = [
        w1*M1^2 + w2*dp^2*((S2+D*M2).')*(S2+D*M2), w2*dp*((S2+D*M2).')*M2;
        w2*dp*((S2+D*M2).')*M2, w2*M2^2
    ];

    R = [
        w1*M1*(S1+D*M1)*qd+w2*dp*((S2+D*M2).')*(S2+D*M2)*qd;
        w2*M2*(S2+D*M2)*qd
    ];
...
save mats/MBPD.mat qs qds qdds ts 
end